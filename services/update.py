...
class UpdateService:
    ...
    def update_book(self, book_id: int, book: BookCreate, user: dict):
        ...
        existing_book = (
            self.session.query(Book).filter(Book.title == book.title).first()
        )
        if existing_book is not None:
            if existing_book.id != results.id:
                raise HTTPException(
                    status_code=409, detail="Book already exist with same title"
                )
    ...
    def update_author(self, author_id: int, author: AuthorCreate, user: dict):
        ...
        """
        results = (
            self.session.query(Author).filter(Author.author_id == author_id).first()
        )
        if not results:
